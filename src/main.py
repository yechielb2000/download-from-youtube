...
def add_elements(root: Tk) -> None:
	...
	button = Button(root, text="Download",  background=YELLOW_COLOR,
					foreground=BLACK_COLOR, font=BOLD, command=lambda:thread(root, entry.get()))
...
def thread(root: Tk, url: str):
	...
	threading.Thread(target=download_youtube_video, args=(url, downloading_message)).start()
...
def download_youtube_video(url: str, downloading_message: Label, *args) -> None:
	try:
		...
		messagebox.showerror(title='Download failed!', message=f'{e}')
